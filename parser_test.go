...
var EntitasLangSource1 = `target entitas_csharp
...
func BenchmarkNewParser(b *testing.B) {
	for n := 0; n < b.N; n++ {
		NewParser(strings.NewReader(EntitasLangSource1))
	}
}

func BenchmarkParseIdentifier(b *testing.B) {
	for n := 0; n < b.N; n++ {
		p := NewParser(strings.NewReader("my_id_hello"))
		p.parseIdentifier()
	}
}
...
func BenchmarkParseTarget(b *testing.B) {
	for n := 0; n < b.N; n++ {
		p := NewParser(strings.NewReader("target my_target"))
		p.parseTargetDecl()
	}
}

func BenchmarkParseEntitasLang(b *testing.B) {
	for n := 0; n < b.N; n++ {
		NewParser(strings.NewReader(EntitasLangSource1)).Parse()
	}
}
